...
require 'D:\RubyProject\IndividualProject\Controllers\TreatmentsTab\update_treatment_controller.rb'

class UpdateTreatmentView
  include Glimmer
  ...
  def initialize(selected_treatment, senior_controller)
    @controller = UpdateTreatmentController.new(self, selected_treatment, senior_controller)
    @required_fields = {
      diagnosis: true,
      cost: true,
      treatment_date: true,
  ...
  def display
    @update_window = window('Изменить запись', 600, 200) {
      margined true
      grid{
        padded true
