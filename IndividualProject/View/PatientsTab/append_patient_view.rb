...
require 'D:\RubyProject\IndividualProject\Controllers\PatientTab\append_patient_controller.rb'

class AppendPatientView
  include Glimmer
  ...
  def initialize(main_view, senior_controller)
    @controller = AppendPatientController.new(self, main_view, senior_controller)
    @required_fields = {
      last_name: false,
      first_name: false,
      patronymic: false,
  ...
  def display
    @append_window = window('Добавить пациента', 800, 200) {
      margined true
      grid {
        label('Фамилия') {
          top 1
