require 'D:\RubyProject\IndividualProject\Controllers\DoctorsTab\append_doctor_controller.rb'
require 'glimmer-dsl-libui'

class AppendDoctorView
  include Glimmer

  NAME = /(^[А-Я][а-я]+$)|(^[A-Z][a-z]+$)/

  def initialize(main_view, senior_controller)
    @controller = AppendDoctorController.new(self, main_view, senior_controller)
    @required_fields = {
      last_name: false,
      first_name: false,
      patronymic: false,
  ...
  def display
    @append_window = window('Добавить специалиста', 800, 300) {
      margined true
      grid {
        label('Фамилия') {
          top 1
